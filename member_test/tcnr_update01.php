			...
			$(function(){
				...
				$("#btn_update").bind("click",function(){
					...
					if(flag_username&&flag_password&&flag_password_check){
							$.ajax({
								type: "POST",
								url: "http://192.168.60.101/member_test/memberAPI/tcnr_member_api.php?query_type=update",
								data:{username:$("#username").val(),password:$("#password").val(),email:$("#email").val(),<?php if($_SESSION['usertype']=="a"): ?>usertype:$("#usertype").val(),<?php endif; ?>id:<?php echo $ID ?>},
								success: show,
								error:function(){
									alert("連接資料庫失敗");
								}
